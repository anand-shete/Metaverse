...
Establish the quality bar that makes architectural changes reviewable. Local Docker Compose (`compose.yml`) runs MongoDB + API via `docker compose up`; production deploy remains `apps/api/compose.yml` + `CD-api.yml`.
...
Root [`compose.yml`](../compose.yml) starts MongoDB 7 + the API image from [`apps/api/Dockerfile`](../apps/api/Dockerfile).

| Command | What it runs |
| ------- | ------------ |
| `docker compose up --build` | API on `:3000`, Mongo on host `:27018` |
| `docker compose up mongo` | Mongo only — pair with `pnpm dev:api` for hot-reload |

- Compose overrides `MONGO_URI` to `mongodb://mongo:27017/metaverse` inside the API container
- Production stack unchanged: [`apps/api/compose.yml`](../apps/api/compose.yml) (ECR image + Redis)
- Guide: [`docs/local-dev.md`](local-dev.md)
...
gantt
  ...
  section Foundation
    ...
    Local Docker Compose        :done, p1c, after p1b, 2d
  section Backend
    Auth + Mongo health         :done, p2a, after p1c, 4d
  section Media
